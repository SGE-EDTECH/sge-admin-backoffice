...
interface BarChartCardProps {
  ...
  data: Array<{ name: string; Ocupação: number; Previsto: number }>;
  color: string;
}

const BarChartCard: React.FC<BarChartCardProps> = ({ title, goal, current, achieved, data, achievedPercentage, color }) => {
  return (
    <div className="card">
      ...
      <div className="chart">
        <ResponsiveContainer width="100%" height={200}>
          <BarChart data={data}>
            ...
            <Tooltip />
            <Legend />
            <Bar dataKey="Ocupação" fill={color} />
            <Bar dataKey="Previsto" fill="#ccc" />
          </BarChart>
        </ResponsiveContainer>
      </div>
      <style jsx>{`
